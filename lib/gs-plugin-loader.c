 ...
 *
 * This function is intended to be used by internal vstore code.
 *
 * Since: 42
 */
void
...
add_app_to_install_queue (GsPluginLoader *plugin_loader, GsApp *app)
{
	...
	g_source_set_name (source, "[vstore] emit_pending_apps_idle");
				   ...
				   GsPluginLoader *plugin_loader)
{
	...
	/* plugin now allowing vstore to show updates panel */
	if (allow_updates) {
					...
					GsPluginLoader *plugin_loader)
{
	...
	gs_plugin_loader_app_create_async (plugin_loader, "system/*/*/org.vaxp.vstore.desktop/*",
...
finish_setup_get_bus (GTask *task)
{
	...
	if (plugin_loader->locations->len == 0) {
		...
		g_ptr_array_add (locations, g_build_filename (LIBDIR, "vstore", filename, NULL));
	} else {
...
gs_plugin_loader_class_init (GsPluginLoaderClass *klass)
{
	 ...
	 *
	 * Whether updates and upgrades are managed by vstore.
	 *
 ...
 *
 * This is intended to be used by internal vstore code. Plugin and UI
