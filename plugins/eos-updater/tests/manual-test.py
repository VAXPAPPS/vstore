...
class ManualTest(dbusmock.DBusTestCase):
    '''A manual test of the eos-updater plugin in vstore.

    It creates a mock eos-updater D-Bus daemon, on the real system bus (because
    otherwise vstore’s other plugins can’t communicate with their
    system daemons; to fix this, we’d need to mock those up too). The test
    harness provides the user with instructions about how to run vstore
    ...
    cancellation of the daemon from vstore; and the daemon unexpectedly
    ...
    def setUp(self):
        ...
        # vstore needs to access various other services (such as
    ...
    def launch_gnome_software(self):
        '''Instruct the user to launch vstore'''
        print('Launch vstore with:')
        print('vstore --verbose')
        self.manual_check('Press enter to continue')

    def kill_gnome_software(self):
        '''Instruct the user to kill vstore'''
        print('Kill vstore with:')
        print('pkill vstore')
        self.manual_check('Press enter to continue')
    ...
    def test_poll_eos_updater_dies(self):
        '''Test that vstore recovers if eos-updater dies while
        polling for updates.'''
        self.dbusmock.SetPollAction(
            'update', dbus.Dictionary({}, signature='sv'), '', '')
