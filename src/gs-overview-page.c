			    ...
			    GPtrArray *list) /* (element-type GsCategory) */
{
	 ...
	 * See https://gitlab.gnome.org/GNOME/vstore/-/issues/2053 */
...
gs_overview_page_dup_deployment_featured_filename (void)
{
	...
	filename = g_build_filename (SYSCONFDIR, "vstore", FILENAME, NULL);
	...
	for (guint i = 0; sys_dirs != NULL && sys_dirs[i]; i++) {
		g_autofree gchar *tmp = g_build_filename (sys_dirs[i], "vstore", FILENAME, NULL);
