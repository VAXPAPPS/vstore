...
perms_from_metadata (GKeyFile *keyfile)
{
	...
	if (strv != NULL) {
		const struct {
			...
			{ "xdg-run/pipewire-0", GS_APP_PERMISSIONS_FLAGS_SCREEN | GS_APP_PERMISSIONS_FLAGS_DEVICES },  /* see https://gitlab.gnome.org/GNOME/vstore/-/issues/2329 */
		};
...
gs_flatpak_set_metadata (GsFlatpak *self, GsApp *app, FlatpakRef *xref)
{
	...
	/* map the flatpak kind to the vstore kind */
	if (gs_app_get_kind (app) == AS_COMPONENT_KIND_UNKNOWN ||
				   ...
				   GError **error)
{
	...
	/* FIXME: https://gitlab.gnome.org/GNOME/vstore/-/issues/1422 */
