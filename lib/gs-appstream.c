	...
	for (g_autoptr(XbNode) child = xb_node_get_child (relation_node); child != NULL; node_set_to_next (&child)) {
		...
		if (g_str_equal (item_kind, "control")) {
			...
			if (kind == AS_RELATION_KIND_REQUIRES &&
			    ...
			    g_strcmp0 (xb_node_get_text (child), "org.vaxp.vstore.desktop") == 0) {
				/* is compatible */
		     ...
		     GError **error)
{
	...
	const Query queries[] = {
		 ...
		 *
		 * See https://gitlab.gnome.org/GNOME/vstore/-/issues/2277 */
			    ...
			    GError **error)
{
	...
	if (xml == NULL) {
		 ...
		 * See https://gitlab.gnome.org/GNOME/vstore/-/merge_requests/763
...
gs_appstream_component_add_extra_info (XbBuilderNode *component)
{
	...
	/* add the vstore-specific 'Addon' group and ensure they
