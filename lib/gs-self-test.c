...
gs_plugin_download_rewrite_func (void)
{
	...
	const gchar *resource = "background:\n"
				" url('file://" DATADIR "/vstore/featured-maps.png')\n"
				" url('file://" DATADIR "/vstore/featured-maps-bg.png')\n"
				" bottom center / contain no-repeat;\n";

	/* only when installed */
	if (!g_file_test (DATADIR "/vstore/featured-maps.png", G_FILE_TEST_EXISTS)) {
...
gs_app_func (void)
{
	...
	app = gs_app_new ("vstore.desktop");
	g_assert (GS_IS_APP (app));
	g_assert_cmpstr (gs_app_get_id (app), ==, "vstore.desktop");
...
main (int argc, char **argv)
{
	...
	/* tests go here */
	g_test_add_func ("/vstore/lib/utils{url}", gs_utils_url_func);
	g_test_add_func ("/vstore/lib/utils{wilson}", gs_utils_wilson_func);
	g_test_add_func ("/vstore/lib/utils{error}", gs_utils_error_func);
	g_test_add_func ("/vstore/lib/utils{cache}", gs_utils_cache_func);
	g_test_add_func ("/vstore/lib/utils{append-kv}", gs_utils_append_kv_func);
	g_test_add_func ("/vstore/lib/os-release", gs_os_release_func);
	g_test_add_func ("/vstore/lib/app", gs_app_func);
	g_test_add_func ("/vstore/lib/app/progress-clamping", gs_app_progress_clamping_func);
	g_test_add_func ("/vstore/lib/app{addons}", gs_app_addons_func);
	g_test_add_func ("/vstore/lib/app{unique-id}", gs_app_unique_id_func);
	g_test_add_data_func ("/vstore/lib/app{thread}", debug, gs_app_thread_func);
	g_test_add_func ("/vstore/lib/app{list}", gs_app_list_func);
	g_test_add_func ("/vstore/lib/app{list-wildcard-dedupe}", gs_app_list_wildcard_dedupe_func);
	g_test_add_func ("/vstore/lib/app{list-performance}", gs_app_list_performance_func);
	g_test_add_func ("/vstore/lib/app{list-related}", gs_app_list_related_func);
	g_test_add_func ("/vstore/lib/plugin", gs_plugin_func);
	g_test_add_func ("/vstore/lib/plugin{download-rewrite}", gs_plugin_download_rewrite_func);
