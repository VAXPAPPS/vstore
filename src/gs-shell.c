...
{
	g_autoptr(GError) error = NULL;
	if (!g_spawn_command_line_async (LIBEXECDIR "/vstore-restarter", &error))
...
gs_shell_main_window_mapped_cb (GtkWidget *widget, GsShell *shell)
{
	 ...
	 * since we only want to hold the scheduler open while the vstore
...
gs_shell_show_event_fallback (GsShell *shell, GsPluginEvent *event)
{
	...
	if (g_error_matches (error, GS_PLUGIN_ERROR, GS_PLUGIN_ERROR_DOWNLOAD_FAILED)) {
		...
		/* TRANSLATORS: failure text for the in-app notification, where the 'Software' means this app, aka 'vstore'. */
...
gs_shell_add_about_menu_item (GsShell *shell)
{
	...
	menu_item = g_menu_item_new (_("About Vstore"), "app.about");
