							...
							GsDetailsPage *self)
{
	...
	if (gs_app_get_license_is_free (self->app)) {
		...
		if (gs_app_get_license (self->app) == NULL) {
			uri = "https://gitlab.gnome.org/GNOME/vstore/-/wikis/software-metadata#license";
		} else {
			...
			if (license_url != NULL && *license_url != '\0') {
				 ...
				 * software. This is a page on the vstore wiki for now,
				...
				uri = "https://gitlab.gnome.org/GNOME/vstore/-/wikis/Software-licensing";
			}
		}
	}
                                   ...
                                   gpointer user_data)
{
	 ...
	 * https://gitlab.gnome.org/GNOME/vstore/-/issues/1192
	 *
